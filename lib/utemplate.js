...
Template = function (element) {
  ...
  function query(object, property) {
    ...
    reduce = function reduce(source, key) {
      return source[key];
    };

    return property.split(regex).filter(filter).reduce(reduce, object);
  }
    ...
    this.sources.map(function (source) {
      this.pipeline.reduce(function (previous, current) {
        ...
        if (typeof current.childNodes !== 'undefined') {
          var setRendered = function setRendered(prepared, children) {
            var child;

            for (var i = 0; i < children.length; i++) {
              if (_typeof((child = children[i]).rendered) !== undefined) {
  ...
  prototype.pipe = function pipe(flow) {
    ...
    switch (typeof flow === 'undefined' ? 'undefined' : _typeof(flow)) {
      case 'undefined':
        return this;
